...
def Generator(input_dim, output_dim, feature_size) -> tf.keras.models.Model:
    ...
    model.add(GRU(units=128,
                  #return_sequences=True,NOT REQUIRED
                  recurrent_dropout=0.02,
                  recurrent_regularizer=regularizers.l2(1e-3)))
    #model.add(Dense(128,
    #              kernel_regularizer=regularizers.l2(1e-3))) IGNORE
...
class GAN():
    ...
    def train(self, X_train, y_train, yc, epochs):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 15 == 0:
                tf.keras.models.save_model(generator, 'WGAN_model_%d.h5' % epoch)
...
mse = MeanSquaredError()
RMSE = np.sqrt(mse(predicted, real))
print('RMSE: ', RMSE)
